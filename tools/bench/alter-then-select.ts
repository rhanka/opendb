// ALTER TABLE + SELECT micro-benchmark.
//
// Stands up a fresh opendb-node, applies N `ALTER TABLE ADD COLUMN`
// statements in sequence, then runs M SELECT-by-PK probes, and emits a
// JSON line summarizing p50 / p95 latencies. Mirrors the jsonb script
// in structure; reuses the same opt-in `--with-pg` flag.
...
function parseOptions(argv: string[]): Options {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (arg === "--rows") {
      ...
      console.log("usage: bench/alter-then-select.ts [--rows N] [--with-pg]");
      process.exit(0);
    } else {
  ...
  return new Promise((resolve, reject) => {
    ...
    server.listen(0, "127.0.0.1", () => {
      ...
      server.close((closeError) => {
        if (closeError === undefined) resolve(port);
        else reject(closeError);
      });
    });
  });
}
...
async function runEngineBench(
  ...
  try {
    ...
    const tableName = `bench_alter_${Date.now()}_${process.pid}`;
    await runQuery(state, `DROP TABLE IF EXISTS ${tableName}`).catch(() => {});
    await runQuery(state, `CREATE TABLE ${tableName} (id INT PRIMARY KEY, label TEXT)`);
    await runQuery(state, `INSERT INTO ${tableName} (id, label) VALUES (1, 'seed')`);

    const alterSamples: number[] = [];
    const alterStart = performance.now();
    for (let i = 0; i < rows; i += 1) {
      const stmt = `ALTER TABLE ${tableName} ADD COLUMN col_${i} TEXT DEFAULT 'x'`;
      const t0 = performance.now();
      await runQuery(state, stmt);
      alterSamples.push(performance.now() - t0);
    }
    const alterTotal = performance.now() - alterStart;

    const selectSamples: number[] = [];
    const selectStart = performance.now();
    for (let i = 0; i < rows; i += 1) {
      const stmt = `SELECT * FROM ${tableName} WHERE id = 1`;
      const t0 = performance.now();
      await runQuery(state, stmt);
    ...
    return {
      engine,
      rows,
      alter_p50_ms: percentile(alterSamples, 0.5),
      alter_p95_ms: percentile(alterSamples, 0.95),
      alter_total_ms: alterTotal,
...
async function spawnOpenDbNode(): Promise<{
  ...
  const dataDir = mkdtempSync(join(repoRoot, "tmp", "bench-alter-"));
  const child = spawn(
    nodeBin,
    [
      "--node-id",
...
async function main(): Promise<void> {
  ...
  process.stdout.write(`${JSON.stringify({ run: "alter-then-select", summaries })}\n`);
}
