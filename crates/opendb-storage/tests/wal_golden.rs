...
#[tokio::test]
async fn wal_reads_frame_v1_record_v2_alter_table_fixture() {
    use opendb_storage::commit_stream::AlterTableOp;
    let temp_dir = tempfile::tempdir().expect("create temp dir");
    let path = temp_dir.path().join("commit.wal");
    let bytes = decode_hex(include_str!(
        "fixtures/wal/frame-v1-record-v2-alter-table.hex"
    ));
    assert_frame(&bytes);
    ...
    let records = Wal::new(&path).read_all().await.expect("read fixture");
    assert_eq!(records.len(), 1);
    let Mutation::AlterTable { table, op } = &records[0].mutations[0] else {
        panic!("expected AlterTable");
    };
    assert_eq!(table, "accounts");
    let AlterTableOp::AddColumn(column) = op else {
