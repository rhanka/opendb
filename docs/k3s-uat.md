...
Sprint 7 adds JSONB columns. Use `JSON` or `JSONB` as a type token; default literals like `'{}'::jsonb` are stripped of their cast suffix and stored as parsed JSON. The pgwire RowDescription emits OID 3802 (JSONB). JSON operators (`->`, `->>`, `@>`) are out of scope until a consumer requires them. The first throughput micro-benchmark lives at `tools/bench/jsonb.ts` (`npm run bench:jsonb -- --rows 1000`), with an opt-in `--with-pg` flag for a side-by-side comparison against a local PostgreSQL reachable via `PGHOST`/`PGPORT`. Bench output is a single JSON line under stdout.

Sprint 8 adds `ALTER TABLE ADD COLUMN`, `ALTER TABLE DROP COLUMN`, `ALTER TABLE RENAME COLUMN`, `ALTER TABLE ADD CONSTRAINT ... FOREIGN KEY ... REFERENCES ... ON DELETE ...`, `ALTER TABLE ADD CONSTRAINT ... UNIQUE`, `CREATE [UNIQUE] INDEX [IF NOT EXISTS] ... USING btree (...)`, and `DO $$ BEGIN ... EXCEPTION WHEN duplicate_object THEN null; END $$;` idempotence blocks. Foreign keys and unique constraints are recorded as metadata but not enforced at this stage (Sprint 9). Index columns are recorded but no acceleration is wired (Sprint 10+). `npm run bench:alter -- --rows 100` exercises the alter-then-select path with the same `--with-pg` toggle.
