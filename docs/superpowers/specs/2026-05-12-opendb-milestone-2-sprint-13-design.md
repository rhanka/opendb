...
Status: draft (2026-05-12) — **user intervention expected from this
sprint forward** (see roadmap).
...
Concrete asks for the next interactive turn:

- Confirm the table subset (or pick a different one).
- Confirm whether Drizzle "simple" mode is acceptable, or if Sprint 12
  Extended pgwire is required first.
- Confirm where the entropiq compose harness lives (local docker
  compose vs k3d apply).
