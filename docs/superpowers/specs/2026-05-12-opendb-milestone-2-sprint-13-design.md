# OpenDB Milestone 2 Sprint 13 Design — Sentropic POC read-only

Status: **blocked on Sprint 12 + Sprint 12.1** (2026-05-13). Smoke
ran against opendb-node and produced a complete decision matrix —
see `docs/bench/sentropic-poc-2026-05-13.md`. Reproduce with
`npm run poc:sentropic:smoke`.

## Goal

Connect a real Drizzle-backed sentropic slice to opendb-node and prove
the read path end to end. The target is one realistic table (`folders`
or `initiatives` — both top-5 in sentropic's query frequency analysis)
served from opendb-node, read by the sentropic HTTP API in a docker
compose harness.
...
2. **Schema subset**: pick 1-5 sentropic tables that have only types
...
4. **Seed data**: replay the first ~100 rows of each chosen table from
   a one-shot SQL dump.
5. **HTTP smoke**: stand up `sentropic/api` against `DATABASE_URL`
...
6. **Report**: capture the trace of every SQL OpenDB sees, surface
   ...
   `docs/bench/sentropic-poc-YYYY-MM-DD.md`.

## Non-Goals

- No write traffic from sentropic (Sprint 14).
...
- Drizzle mode: try simple-query first via the smoke
  (`tools/sentropic-poc/smoke.ts`) — Sprint 12 reactivated if the smoke
  proves Extended is unavoidable.
- Compose harness: local `docker-compose.yml` under
  `tools/sentropic-poc/` (single opendb-node + sentropic-api service);
  k3d kept for the Sprint 5+ cluster track.

## Findings 2026-05-13 (smoke run)

The probe matrix is in `docs/bench/sentropic-poc-2026-05-13.md`. Summary:
...
- Conclusion: Sprint 13 cannot start until Sprint 12 (Extended) and a
  ...
  that, the smoke under `npm run poc:sentropic:smoke` is the green-
  light gate before adding the sentropic compose harness and HTTP
  smoke.
