...
| Q5 | FAIL | `select "folders"."id", "folders"."name", "folders"."status", count("initiatives"."id") from "folders" left join "initiatives" on ("initiatives"."folder_id" = "folders"."id" and "initiatives"."workspac` | error: sql error: invalid ON expression: ("initiatives"."folder_id" = "folders"."id" and "initiatives"."workspace_id" = 'w1') |
...
| Q7 | PASS | `select "id", "owner_user_id", "name", "type", "gate_config", "hidden_at", "created_at", "updated_at" from "workspaces" where "workspaces"."id" = $1` | rows=[{"id":"w1","ownerUserId":"u1","name":"Acme","type":"ai-ideas","gateConfig":"","hiddenAt":"2000-01-01T05:00:00.000Z","createdAt":"2026-05-01T00:00:00.000Z","updatedAt":"1970-0 |
